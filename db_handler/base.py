...
async def get_all_not_closed_claims(session):
    ...
    try:
        async with async_session() as session:
            ...
            not_closed_claims:List[Claim] = (await session.execute(select(Claim).where(Claim.status.in_(["В работе", "Ожидает подтверждения", "Требуется доработка", "Срок превышен"])))).scalars().all()
